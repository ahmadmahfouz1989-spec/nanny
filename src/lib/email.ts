...
const FROM = process.env.RESEND_FROM_EMAIL ?? "ouiKnow <onboarding@resend.dev>";
...
export function newSignupAdminEmail(lang: Lang, newUserEmail: string) {
  const addr = escapeHtml(newUserEmail);
  return {
    subject: pick(lang, "New sign-up", "تسجيل جديد"),
    html: pick(
      lang,
      `<p>A new account just signed up: <strong>${addr}</strong>.</p><p>Their profile will show up in the moderation queue once they pick a category and finish onboarding.</p>`,
      `<p>سجّل حساب جديد للتو: <strong>${addr}</strong>.</p><p>سيظهر ملفه في قائمة المراجعة بعد اختيار فئة وإكمال الإعداد.</p>`,
    ),
  };
}
...
export function alreadyRegisteredEmail(lang: Lang, loginUrl: string, recoverUrl: string) {
  return {
    ...
    html: pick(
      lang,
      `<p>Someone just tried to sign up with this email, but you already have a ouiKnow account.</p><p><a href="${loginUrl}">Log in</a> or <a href="${recoverUrl}">reset your password</a> if you forgot it.</p><p>If this wasn't you, you can safely ignore this email.</p>`,
      `<p>حاول شخص ما للتو إنشاء حساب بهذا البريد الإلكتروني، لكن لديك حساب على ouiKnow بالفعل.</p><p><a href="${loginUrl}">سجّل الدخول</a> أو <a href="${recoverUrl}">أعد تعيين كلمة المرور</a> إذا نسيتها.</p><p>إذا لم يكن هذا أنت، يمكنك تجاهل هذا البريد بأمان.</p>`,
    ),
  };
}
...
export function newMessageEmail(lang: Lang, fromName: string, snippet: string, messagesUrl: string) {
  ...
  return {
    ...
    html: pick(
      lang,
      `<p>${name} sent you a message on ouiKnow:</p><blockquote style="margin:0;padding:8px 12px;border-inline-start:3px solid #ddd;color:#555">${body}</blockquote><p><a href="${messagesUrl}">Open the conversation</a> to reply.</p><p style="color:#888;font-size:13px">We'll only email you once per conversation until you've read it.</p>`,
      `<p>أرسل/أرسلت ${name} رسالة إليك على ouiKnow:</p><blockquote style="margin:0;padding:8px 12px;border-inline-start:3px solid #ddd;color:#555">${body}</blockquote><p><a href="${messagesUrl}">افتح المحادثة</a> للرد.</p><p style="color:#888;font-size:13px">سنرسل لك بريدًا واحدًا فقط لكل محادثة حتى تقرأها.</p>`,
    ),
  };
}
...
export function signupConfirmationEmail(lang: Lang, confirmUrl: string) {
  return {
    ...
    html: pick(
      lang,
      `<p>Welcome to ouiKnow — click below to confirm your email and activate your account.</p><p><a href="${confirmUrl}">Confirm email</a></p><p>If you didn't create this account, you can ignore this email.</p>`,
      `<p>مرحبًا بك في ouiKnow — اضغط أدناه لتأكيد بريدك الإلكتروني وتفعيل حسابك.</p><p><a href="${confirmUrl}">تأكيد البريد الإلكتروني</a></p><p>إذا لم تُنشئ هذا الحساب، يمكنك تجاهل هذا البريد.</p>`,
    ),
  };
}
