"use client";

import { useEffect, useState } from "react";
import Image from "next/image";
import { useLocale, useTranslations } from "next-intl";
import ResultsShell from "@/components/matches/results-shell";
import ChatThread from "@/components/matches/chat-thread";
import AvatarIllustration from "@/components/illustrations/avatar-illustration";
import { ui } from "@/lib/ui";

const TONES = ["primary", "secondary", "berry"] as const;

type Conversation = {
  matchId: string;
  counterpart: { name: string; photoUrl: string | null };
  lastMessage: { body: string; createdAt: string } | null;
  unreadCount: number;
};

type ThreadMessage = { id: string; sender_id: string; body: string; created_at: string };

function relativeTime(iso: string, locale: string) {
  const diffMs = Date.now() - new Date(iso).getTime();
  const diffMin = Math.round(diffMs / 60000);
  const rtf = new Intl.RelativeTimeFormat(locale, { numeric: "auto" });
  if (diffMin < 60) return rtf.format(-diffMin, "minute");
  const diffHour = Math.round(diffMin / 60);
  if (diffHour < 24) return rtf.format(-diffHour, "hour");
  const diffDay = Math.round(diffHour / 24);
  return rtf.format(-diffDay, "day");
}

export default function MessagesPage() {
  const t = useTranslations("Inbox");
  const tMatches = useTranslations("Matches");
  const locale = useLocale();
  const [conversations, setConversations] = useState<Conversation[] | null>(null);
  const [expanded, setExpanded] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/messages/inbox")
      .then((res) => res.json())
      .then((body) => setConversations(body.conversations ?? []));
  }, []);

  function handleMessage(matchId: string, message: ThreadMessage) {
    setConversations((cs) => {
      if (!cs) return cs;
      const updated = cs.map((c) =>
        c.matchId === matchId
          ? { ...c, lastMessage: { body: message.body, createdAt: message.created_at } }
          : c,
      );
      return [...updated].sort((a, b) => {
        const at = a.lastMessage ? new Date(a.lastMessage.createdAt).getTime() : 0;
        const bt = b.lastMessage ? new Date(b.lastMessage.createdAt).getTime() : 0;
        return bt - at;
      });
    });
  }

  function toggle(matchId: string) {
    setExpanded((prev) => {
      const next = prev === matchId ? null : matchId;
      if (next) {
        // Optimistically clear the badge for the thread being opened.
        setConversations((cs) => cs?.map((c) => (c.matchId === matchId ? { ...c, unreadCount: 0 } : c)) ?? cs);
      }
      return next;
    });
  }

  return (
    <ResultsShell title={t("title")} backLabel={tMatches("backToDashboard")}>
      {!conversations && <p className="text-sm text-muted">{tMatches("loading")}</p>}
      {conversations && conversations.length === 0 && <p className="text-sm text-muted">{t("empty")}</p>}

      <div className="flex flex-col gap-3">
        {conversations?.map((c, i) => (
          <div key={c.matchId} className={ui.card + " overflow-hidden"}>
            <button
              type="button"
              onClick={() => toggle(c.matchId)}
              className="w-full flex items-center gap-3 p-4 text-start"
            >
              {c.counterpart.photoUrl ? (
                <Image
                  src={c.counterpart.photoUrl}
                  alt=""
                  width={48}
                  height={48}
                  unoptimized
                  className="h-12 w-12 rounded-full object-cover shrink-0"
                />
              ) : (
                <AvatarIllustration tone={TONES[i % TONES.length]} className="h-12 w-12 rounded-full shrink-0" />
              )}
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <p className="font-display font-semibold truncate">{c.counterpart.name}</p>
                  {c.lastMessage && (
                    <span className="text-xs text-muted shrink-0">
                      {relativeTime(c.lastMessage.createdAt, locale)}
                    </span>
                  )}
                </div>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm text-muted truncate">
                    {c.lastMessage ? c.lastMessage.body : t("noMessagesYet")}
                  </p>
                  {c.unreadCount > 0 && (
                    <span className="inline-flex items-center justify-center h-5 min-w-5 px-1.5 rounded-full bg-primary text-white text-xs font-semibold shrink-0">
                      {c.unreadCount}
                    </span>
                  )}
                </div>
              </div>
            </button>
            {expanded === c.matchId && (
              <div className="px-4 pb-4">
                <ChatThread matchId={c.matchId} onMessage={(m) => handleMessage(c.matchId, m)} />
              </div>
            )}
          </div>
        ))}
      </div>
    </ResultsShell>
  );
}
